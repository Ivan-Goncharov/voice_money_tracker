...
class Currencies extends Table {
  ...
  TextColumn get code =>
      text().withLength(min: 3, max: 3)(); // ISO currency code (RUB, USD)
  TextColumn get name => text().withLength(min: 1, max: 100)();
  TextColumn get symbol =>
      text().withLength(min: 1, max: 5)(); // Currency symbol (₽, $)
...
class Expenses extends Table {
  ...
  TextColumn get description =>
      text()
          .withLength(min: 0, max: 500)
          .nullable()(); // Made nullable and min length 0
  IntColumn get categoryId => integer().references(Categories, #id)();
  IntColumn get currencyId =>
      integer()
          .references(Currencies, #id)
          .withDefault(const Constant(1))(); // Default to RUB
...
class CategoriesDao extends DatabaseAccessor<AppDatabase>
    ...
    implements CategoriesDaoInterface {
...
class ExpensesDao extends DatabaseAccessor<AppDatabase>
    ...
    implements ExpensesDaoInterface {
  ...
  Future<bool> updateExpense(int id, ExpensesCompanion expense) async {
    final result = await (update(expenses)
      ..where((tbl) => tbl.id.equals(id))).write(expense);
    return result > 0;
  }
...
class CurrenciesDao extends DatabaseAccessor<AppDatabase>
  ...
  /// Get all currencies
  Future<List<CurrencyEntity>> getAllCurrencies() => select(currencies).get();

  /// Get only active currencies
  Future<List<CurrencyEntity>> getActiveCurrencies() =>
      (select(currencies)..where((tbl) => tbl.isActive)).get();

  /// Get currency by ID
  Future<CurrencyEntity?> getCurrencyById(int id) =>
      (select(currencies)..where((tbl) => tbl.id.equals(id))).getSingleOrNull();
  ...
  Future<bool> updateCurrency(int id, CurrenciesCompanion currency) async {
    final result = await (update(currencies)
      ..where((tbl) => tbl.id.equals(id))).write(currency);
    return result > 0;
  }

  /// Delete a currency
  Future<int> deleteCurrency(int id) =>
      (delete(currencies)..where((tbl) => tbl.id.equals(id))).go();
}
...
// Database
@DriftDatabase(
  tables: [Currencies, Categories, Expenses],
  daos: [CurrenciesDao, CategoriesDao, ExpensesDao],
)
    ...
    return MigrationStrategy(
        ...
        await batch((batch) {
          batch.insertAll(categories, [
            ...
            CategoriesCompanion.insert(
              name: 'Transportation',
              description: const Value(
                'Car, gas, public transport, and travel',
              ),
              color: '#4ECDC4',
            ...
            CategoriesCompanion.insert(
              name: 'Shopping',
              description: const Value(
                'Clothes, electronics, and general shopping',
              ),
              color: '#45B7D1',
            ...
            CategoriesCompanion.insert(
              ...
              description: const Value(
                'Rent, electricity, water, and other bills',
              ),
              color: '#FECA57',
      ...
      onUpgrade: (Migrator m, int from, int to) async {
        if (from == 1 && to == 2) {
          ...
          await m.addColumn(expenses, expenses.currencyId);
        }
      },
    );
  }
